...
VRAM allocation (adaptive, auto-split by Ollama):
  >6 GB VRAM:  full Gemma 3n E2B on GPU + Whisper
  <=6 GB VRAM: partial Gemma 3n E2B on GPU, rest on CPU RAM
...
# Ollama (LLM) - Gemma 3n E2B by default (smaller, lower hardware bar)
...
def init():
    ...
    print('-' * 64)
    print(f'  VRAM budget:')
    print(f'    Gemma 3n E2B (via Ollama) ..... auto-split GPU/CPU')
...
def ensure_model_pulled():
    ...
    print(f'  Pulling {OLLAMA_MODEL} (first run only, ~3 GB)...')
    try:
